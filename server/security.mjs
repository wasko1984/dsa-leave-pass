...
export function validateSignature(signature) {
  ...
  let count = 0, firstPoint = null, distinct = false;
  for (const stroke of signature) {
    ...
    for (const p of stroke) {
      requireThat(Array.isArray(p) && p.length === 2 && p.every(n => Number.isFinite(n) && n >= 0 && n <= 100), 400, 'Invalid signature coordinates.');
      count++;
      if (!firstPoint) {
        firstPoint = p;
      } else if (p[0] !== firstPoint[0] || p[1] !== firstPoint[1]) {
        distinct = true;
      }
    }
  }
  requireThat(count >= 4 && count <= 10000 && distinct, 400, 'Please draw a complete signature.');
  return signature;
}
