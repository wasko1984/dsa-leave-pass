...
export function canView(user, a, db) {
  if (user.role === 'admin' || a.ownerId === user.id) return true;
  if (a.stage === 'draft') {
    if (!a.fields.relieverId) return false;
    if (db) {
      const reliever = db.prepare('SELECT * FROM users WHERE id=? AND active=1').get(a.fields.relieverId);
      return Boolean(reliever && reliever.role === 'staff' && reliever.directorate === a.directorate && reliever.id === user.id);
    }
    return a.fields.relieverId === user.id && user.role === 'staff' && user.directorate === a.directorate;
  }
  return a.fields.relieverId === user.id || (user.role === 'doa' || (user.role !== 'staff' && user.directorate === a.directorate));
}
...
function fieldsFor(db, user, input, submit) {
  ...
  const fields = {};
  for (const key of ['fileNumber', 'placeOfDeployment', 'description', 'previousDate', 'requestThrough', 'effectiveDate', 'reasons', 'travelPlace', 'contactAddress', 'station', 'department', 'relieverId']) {
    fields[key] = clean(input[key], key === 'reasons' || key === 'contactAddress' ? 2000 : 200);
  }
  fields.days = Number(input.days) || 0;
  
  if (fields.relieverId) {
    ...
    fields.relieverName = reliever.name;
    fields.relieverAppointment = reliever.appointment;
  }
  
  if (submit) {
    for (const key of ['fileNumber', 'placeOfDeployment', 'description', 'requestThrough', 'effectiveDate', 'reasons', 'travelPlace', 'contactAddress', 'station', 'department', 'relieverId']) {
      requireThat(fields[key], 400, `Please complete ${key.replace(/([A-Z])/g, ' $1').toLowerCase()}.`);
    }
...
function attachment(db, a, input) {
  ...
  requireThat(input && typeof input.name === 'string' && typeof input.base64 === 'string', 400, 'Attachment is invalid.');
  const rawName = input.name.trim();
  const ext = (rawName.split('.').pop() || '').toLowerCase();
  requireThat(['pdf', 'png', 'jpg', 'jpeg'].includes(ext) && !rawName.toLowerCase().endsWith('.exe'), 400, 'Only PDF, PNG and JPEG attachments are supported.');
  
  requireThat(input.base64.length <= 4200000 && /^[A-Za-z0-9+/]*={0,2}$/.test(input.base64), 400, 'Attachment must be up to 3 MB.');
  ...
  if (bytes.subarray(0, 4).toString('utf8') === '%PDF') {
    ...
    requireThat(ext === 'pdf', 400, 'Attachment extension does not match file content.');
  } else if (bytes.subarray(0, 8).equals(Buffer.from([137, 80, 78, 71, 13, 10, 26, 10]))) {
    detectedMime = 'image/png';
    requireThat(ext === 'png', 400, 'Attachment extension does not match file content.');
  } else if (bytes[0] === 255 && bytes[1] === 216 && bytes[2] === 255) {
    detectedMime = 'image/jpeg';
    requireThat(ext === 'jpg' || ext === 'jpeg', 400, 'Attachment extension does not match file content.');
  }
  requireThat(detectedMime, 400, 'Only PDF, PNG and JPEG attachments are supported.');
  
  const id = randomUUID(), name = clean(rawName, 120).replace(/[^a-zA-Z0-9 ._()-]/g, '_') || 'attachment';
  db.prepare('INSERT INTO attachments(id,application_id,name,mime,content) VALUES(?,?,?,?,?)').run(id, a.id, name, detectedMime, bytes);
  a.attachment = { id, name, mime: detectedMime, size: bytes.length };
}
...
export function saveApplication(db, user, input, id) {
  ...
  return transaction(db, () => {
    ...
    if (input.submit === true) {
      requireThat(input.confirm === true || input.confirm === 'true', 400, 'Please confirm that the information is correct and this is your signature.');
      sign(a, user, 'applicant', input.signature);
    }
...
export function decide(db, user, id, input) {
  return transaction(db, () => {
    ...
    requireThat(canView(user, a, db), 403, 'You do not have access to this application.');
    requireThat(a.version === input.version, 409, 'This application has changed. Refresh and try again.');
